...
//VERSION NOTES:
// First CUDA parallel version, not very fast.
// It's about 5x slower than the CPU version due
// to memory-thread uncoalescence. 
...
__global__ void blackSOR(float *phi, float *T, float omega, float lambda)
{
    ...
    // Use block.Idx.x as 'x', blockIdx.y as 'y' and threadIdx.x as 'z'
    
    // We don't calculate the border values.
    if ( ! ( ( ( blockIdx.x == 0 ) || ( blockIdx.y == 0 ) || ( threadIdx.x == 0 ) ) 
          || ( blockIdx.x == 127 ) || ( blockIdx.y == 127 ) || ( threadIdx.x == 127 ) ) )
    {
        unsigned int incr = ( ( ( blockIdx.y % 2 ) + ( threadIdx.x % 2 ) ) % 2 );
        // Black?
        if ( ( blockIdx.x % 2 )  != incr )
        {
            unsigned int i = blockIdx.y * gridDim.x + threadIdx.x * gridDim.x * gridDim.y + blockIdx.x;
            phi[i] = omega * (
                  T[6 * i    ] * phi[i + 1 ] 
                + T[6 * i + 1] * phi[i - 1 ] 
...
__global__ void redSOR(float *phi, float *T, float omega, float lambda)
{
    ...
    if  ( ! ( ( blockIdx.x == 0 ) || ( blockIdx.y == 0 ) || ( threadIdx.x == 0 )
       || ( blockIdx.x == 127 ) || ( blockIdx.y == 127 ) || ( threadIdx.x == 127 ) ) )
    {
        unsigned int incr = 1 - ( ( ( blockIdx.y % 2 ) + ( threadIdx.x % 2 ) ) % 2 );
        // Red?
        if ( (  blockIdx.x % 2 ) != incr )
        {
            unsigned int i = blockIdx.y * gridDim.x + threadIdx.x * gridDim.x * gridDim.y + blockIdx.x;
