...
#include "/home/cudauser/NVIDIA_CUDA_SDK/common/inc/cutil.h"
...
//Kernel definition
__global__ void doSOR(int Nx, int Ny, int Nz, float *matrix, float *Tmatrix, float *Qmatrix, int *charged_black_points, int *charged_white_points, float *tmp_phi, float *rms_change_dev, float *max_residual_dev, int *iteration_dev)
{
    float *T, *phi;
    float *Q;
...
//          printf("Iteration number: %d\n", iteration);
    ...
    rms_change_dev[0]     = rms_change;//rms_change;
    max_residual_dev[0]   = max_residual;//max_residual;
    iteration_dev[0]      = iteration;//iteration;
}
...
int main(void)
{
    ...
    cudaMalloc( (void **) &Qmatrix_dev, (sizeof(float)*Msize) );
    cudaMemcpy( Qmatrix_dev, Qmatrix, (sizeof(float)*Msize), cudaMemcpyHostToDevice );
...
//    *num_iteration_dev = 6;
    ...
    CUDA_SAFE_CALL( (doSOR <<<1, 1>>> (size[0], size[1], size[2], d_matrix, Tmatrix_dev, Qmatrix_dev, CBPoints_dev, CWPoints_dev, tmp_phi_dev, rms_change_dev, max_residual_dev, num_iteration_dev)) );
    cudaThreadSynchronize();
    printf("Immediately after sync \n");
    ...
    cudaMemcpy( num_iteration, num_iteration_dev, (sizeof(int)), cudaMemcpyDeviceToHost );
    cudaThreadSynchronize();
    ...
    cudaFree(num_iteration_dev);
    cudaFreeHost(rms_change);
    cudaFreeHost(max_residual);
    cudaFreeHost(num_iteration);
