...
{    
    ...
    for (charge_pointer = charged_points;
            charge_pointer < &charged_points[number_of_charged_points];
            charge_pointer++)
    {
...
/*        cudaMemcpy( phi_host, phi, sizeof(float)*Msize, cudaMemcpyDeviceToHost );
        ...
        cudaMemcpy( phi, phi_host, sizeof(float)*Msize, cudaMemcpyHostToDevice );
*/      
        calcCP<<< 1, 1 >>> ( phi, Q, charged_black_points, number_of_charged_black_points, omega );
        cudaThreadSynchronize();
...
/*        cudaMemcpy( phi_host, phi, sizeof(float)*Msize, cudaMemcpyDeviceToHost );
        ...
        //Copy back after changes
*/
        calcCP<<< 1, 1 >>> ( phi, Q, charged_white_points, number_of_charged_white_points, omega );
        cudaThreadSynchronize();
        cudaMemcpy( phi_host, phi, sizeof(float)*Msize, cudaMemcpyDeviceToHost );
