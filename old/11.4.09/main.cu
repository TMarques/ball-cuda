...
//Kernel definition
__global__ void doSOR(int Nx, int Ny, int Nz, float *matrix, float *Tmatrix, int *Qmatrix, int *charged_black_points, int *charged_white_points, float *tmp_phi, float *Hrms_change, float *Hmax_residual, int *Hiteration)
{
    float *T, *phi;
    int *Q;
    ...
    int max_iterations = 1500, iteration = 0, black, white;
...
//          printf("Iteration number: %d\n", iteration);
    ...
    *Hrms_change     = rms_change;
    *Hmax_residual   = max_residual;
...
int main(void)
{
	...
	//Call kernel
	doSOR <<<1, 1>>> (size[0], size[1], size[2], d_matrix, Tmatrix_dev, Qmatrix_dev, CBPoints_dev, CWPoints_dev, tmp_phi_dev, &rms_change, &max_residual, &num_iteration);
